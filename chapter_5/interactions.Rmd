...
```{r, echo=FALSE, fig.width=8, fig.height=3, warning=FALSE}
model <- model.dem.int
# model <- model.dem.simple
# pred.probs <- model$model %>% select(-1) %>%
#   bind_cols(data.frame(predict(model, ., type="link", se=TRUE))) %>%
#   mutate(pred.prob = model$family$linkinv(fit),
#          pred.lower = model$family$linkinv(fit - (std * se.fit)),
#          pred.upper = model$family$linkinv(fit + (std * se.fit)))
# predict(model)
# plot.data <- pred.probs %>%
#   group_by(inreport1, fh_cl1) %>%
#   summarize(fit = mean(fit))

pred.probs <- augment(model) %>% 
  mutate(pred.prob = model$family$linkinv(.fitted),
         pred.lower = model$family$linkinv(.fitted - (std * .se.fit)),
         pred.upper = model$family$linkinv(.fitted + (std * .se.fit))) %>%
  group_by(inreport1, fh_cl1) %>%
  summarize(pred.prob = mean(pred.prob))

ggplot() + 
  geom_bar(data=prob.crim.democracy.report, 
           aes(x=fh_cl1, y=prob.yes, fill=in.report), 
           alpha=0.5, stat="identity", position="dodge") + 
  geom_line(data=pred.probs, 
             aes(x=fh_cl1, y=pred.prob, colour=as.factor(inreport1)),
             alpha=1)
```
